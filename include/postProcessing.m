...
if (fid > 0)
    ...
    if ~settings.INSAT
        ...
        trackResults = load([settings.dir '\trkResults.mat']).trackResults;
        % [navSolutions, eph, svTimeTable,activeChnList] = postNavigation_sxy(trackResults, settings);
        [navSolutions, eph] = postNavigationNomial(trackResults, settings);
        ...
        [INSATsetting,Rx,trackResults] = INSAT(fid, channel,trackResults,navSolutions,eph,activeChnList,svTimeTable, settings);
        imuInfo = load('texbatCleanDynamic.mat').texbatCleanDynamic;
        ...
        plotINSAT(Rx,imuInfo,navSolutions,trackResults,settings)
        fclose all;
        return;     
    end
